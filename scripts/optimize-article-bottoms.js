 ...
 * 2. Replace [Gtbuy Spreadsheet](docs.google.com) -> [Gtbuy Spreadsheet](https://usfanslinki.com/)
...
function processArticle(filePath) {
  ...
  // Step 2: Check for existing Gtbuy Spreadsheet anchor (case-insensitive)
  const ksRegex = /\[(gtbuy spreadsheet)\]\(([^)]+)\)/gi;
  ...
  if (!hasKsAnchor) {
    ...
    const summaryNoLink = summary.replace(
      'visit the [Gtbuy Spreadsheet](https://usfanslinki.com/) homepage',
      'visit the Gtbuy Spreadsheet homepage'
    );
    content = content.trimEnd() + '\n\n' + summaryNoLink + '\n';
  }
